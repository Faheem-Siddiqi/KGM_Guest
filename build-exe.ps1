...
function Test-MySqlPort {
    ...
    try {
        ...
        if (-not $success) {
            $client.Close()
            Write-Warning "MySQL check timed out after $TimeoutMs ms. Continuing build..."
            return $false
        }
        ...
        Write-Warning "MySQL not reachable on ${HostName}:${Port}. Continuing build..."
        return $false
    }
}
...
try {
    ...
    if (-not (Test-Path -LiteralPath $envFile -PathType Leaf)) {
        throw ".env was not found in the project root: $envFile"
    }
    Check-MySqlReachable -EnvFile $envFile

    Write-Step -Message "Preparing target folder before Maven package" -Percent 34
    if ($CleanTarget) {
    ...
    Write-Step -Message "Running jpackage to create KGM.exe app image" -Percent 62
    $jPackageWorkRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("kgm-jpackage-build-" + [System.Guid]::NewGuid().ToString("N"))
    ...
    Write-Step -Message "Creating output folders without touching user data" -Percent 74
    ...
    Write-Step -Message "Backing up old KGM.exe, app, and runtime if present" -Percent 82
    ...
    Write-Step -Message "Preserving data folders and finishing package" -Percent 97
    Copy-DirectoryIfMissing -Source $imagesSource -Destination (Join-Path $resolvedOutput "images")
    New-Item -ItemType Directory -Path (Join-Path $resolvedOutput "images\uploads") -Force | Out-Null
    Copy-DirectoryIfMissing -Source (Join-Path $projectRoot "employees") -Destination (Join-Path $resolvedOutput "employees")
    $targetEnv = Join-Path $resolvedOutput "config\.env"
    ...
    Write-Step -Message "Build complete" -Percent 100
    Complete-Progress
    Write-Host ""
    Write-Host "SUCCESS: KGM app package is ready."
    Write-Host "Output: $resolvedOutput"
    Write-Host "Run: $resolvedOutput\KGM.exe"
